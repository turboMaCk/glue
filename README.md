...
This package helps you to reduce boilerplate while composing TEA (The Elm Architecture) based application using [`Cmd.map`](http://package.elm-lang.org/packages/elm-lang/core/5.1.1/Platform-Cmd#map), [`Sub.map`](http://package.elm-lang.org/packages/elm-lang/core/5.1.1/Platform-Sub#map) and [`Html.map`](http://package.elm-lang.org/packages/elm-lang/html/2.0.0/Html#map).
...
It's fair to say that `Component` is alsternative to [elm-parts](http://package.elm-lang.org/packages/debois/elm-parts/latest)
...
- Make it possible to use with [`Polymorphic Components`](#polymorphic-component).
- Support [`Action Bubbling`](#action-bubling) from child component to parent one.
...
but tends to grow pretty quickly in unmanagble way. Also not everyone believes that keeping so much stuff in few giant blobs is good way
to organize every application. One other example might be incremental rewrite of application to elm where you need need to build new application
from small parts that can be idepently integrated to logacy codebase. This is when [`Cmd.map`](http://package.elm-lang.org/packages/elm-lang/html/2.0.0/Html#map)
and [`Html.map`](http://package.elm-lang.org/packages/debois/elm-parts/latest) can become handy. You can then start nesting TEA components in TEA components.
Your app is one big component composed by smaller ones which can be composed by more components as well.
...
Then you can use `Cmd.map` inside `init` and `Sub.map` in `subscritions` to finish integration of sub-component to upper one.

**And this is as far as pure TEA can goes. This can be possibly enough for you and that's ok. Why you might need `Component`?**

- This approach requires a lot of bailerplate code inside `update` `init` and `subscriptions`.
- On all places you're handling both parent and child component.
- Many changes in SubComponends requires changes in its parrent as well.
- You have to be really careful to work with subComponent from its parrent (isses with missed subscriptions etc.)
...
To simplify this conpect as well as making components definition easier `Component` type is introduced. Same way `Html.Program` glues TEA together
`Component.Component` glues component functions. Unlike `Program` `Component` is in fact just bunch of functions describing glue between parent and children.
Other functions within `Component` package then using functions this type holds. Thanks to this all glueing is kept on one place.
...
```
All maping from one type to another happens in here. This is diffrent in case of [polymorfic components](#wrap-polymorfic-component).

With `Component` define we can go and integrate it to parent.

Before we do so this is how parrent's `Model` and `Msg` looks like:
...
Polymophic component is what I call TEA components that are already designed to be integrated to some other app.
This basically means they are using `Cmd.map`, `Html.map` and `Sub.map` internaly. Let's make `Counter.elm` from examples polymorfic.
...
Please check [cmd-extra](http://package.elm-lang.org/packages/GlobalWebIndex/cmd-extra/latest) packge
